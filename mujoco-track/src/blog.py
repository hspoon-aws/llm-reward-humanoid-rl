...
class BlogAssembler:
    ...
    _ISAAC_SETUP_DEFAULT = (
        "Physical-AI simulation runs in **MuJoCo MJX** (JAX) on the MuJoCo "
        "Playground Unitree H1 locomotion task, reframed from gait/velocity "
        "tracking into **point-to-point goal-reaching**: the robot starts at "
        "point A and must reach a configurable Goal (point B) within a success "
        "radius while staying upright. A goal-conditioned observation is appended "
        "to the proprioceptive policy input, the LLM-generated reward (JAX) shapes "
        "the policy via the Eureka-style LLM→Reward→RL loop, and PPO training runs "
        "with **Brax** on the B200. This is the MuJoCo track of a dual-simulator "
        "study; the sibling Isaac Lab track runs the same task on an RTX instance."
    )
    ...
    def __init__(
        ...
        *,
        ...
        title: str = "Teaching a Humanoid to Walk: A Self-Hosted LLM + MuJoCo MJX Learning Log",
    ...
    def assemble(
        ...
        *,
        ...
        sections.append(self._render_heading("Running MuJoCo MJX Simulation", level=2))
